...
import {
  ...
  type AiObjectiveChatPanelProps,
...
interface AiAgentDrawerProps {
  ...
  /** Callbacks para el panel de objetivos. Solo se usan cuando context="objectives". */
  objectiveCallbacks?: Omit<AiObjectiveChatPanelProps, "onClose">;
}
...
export function AiAgentDrawer({ open, onOpenChange, context = "dashboard", objectiveCallbacks }: AiAgentDrawerProps) {
  const [prompt, setPrompt] = React.useState("");
  const inputRef = React.useRef<HTMLTextAreaElement>(null);
  const firstName = CURRENT_USER?.name || "Usuario";

  const isObjectives = context === "objectives";

  const config = isObjectives
    ? { title: "Crear Objetivos con IA", placeholder: "", suggestions: [] }
    : CONTEXT_CONFIG[context];

  return (
    <ShellAgentPanelSlot>
      {/* Outer wrapper is what actually animates and narrows the content
      ...
      >
          {isObjectives && objectiveCallbacks ? (
            ...
            <div
              className={cn(
                // Same rounded-card treatment as the sidebar it sits beside
                "flex h-full flex-col overflow-hidden rounded-2xl border border-border/60 bg-ai-mesh-agent shadow-card",
                !open && "invisible"
              )}
