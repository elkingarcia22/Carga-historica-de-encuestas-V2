...
import { formatCicloDate, parseCicloDate } from "./cicloListDates";

/**
 * The closing date of one ciclo, while "Editar fechas" is changing it.
 * Mirrors `survey-list/SurveyDateCell`, trimmed to the one thing the ciclo
 * rail's "Editar fechas" does: move the closing date.
 */
export function CicloDateCell({
  value,
  minDate,
  onSave,
  onCancel,
}: {
  /** The row's current fecha de cierre, in the list's own format. */
  value: string;
  minDate: Date;
  onSave: (date: Date) => void;
  onCancel: () => void;
}) {
  const [staged, setStaged] = React.useState<Date | undefined>(() => parseCicloDate(value) ?? undefined);
    ...
    >
      <PopoverTrigger asChild>
        ...
        >
          ...
          <span className={staged ? undefined : "text-muted-foreground font-medium"}>
            {staged ? formatCicloDate(staged) : "Elegir"}
          </span>
        </button>
      </PopoverTrigger>
      ...
      >
        <div className="border-b border-border/60 px-3 py-2.5">
          <p className="text-[13px] font-bold tracking-tight text-text-primary">Nueva fecha de cierre</p>
          <p className="text-[11px] text-muted-foreground">El ciclo deja de admitir avances ese día.</p>
        </div>
